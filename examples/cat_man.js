...
async function restoreCats (apps, options) {
  ...
  try {
    apps = steam.loginusers.users[ options.id64 ].sharedconfig.UserRoamingConfigStore.Software.Valve.Steam.Apps
...
async function run () {
  try {
    ...
    options.id64 = '' + steam.currentUser.id64

    await steam.load(steam.paths.sharedconfig)

    let sc = Object.assign({}, steam.loginusers.users[ options.id64 ].sharedconfig.UserRoamingConfigStore.Software.Valve.Steam.Apps)
