...
const steamUtils = require('../lib/steamdata-utils.js')
...
async function run () {
  try {
    ...
    if (!options.user) {
      ...
      options.user = Object.assign({}, steam.currentUser)
    } else {
    ...
    let sc = Object.assign({}, steam.loginusers.users[ steam.currentUser.id64 ].sharedconfig.UserRoamingConfigStore.Software.Valve.Steam.Apps)
    ...
    steam.loginusers.users[ steam.currentUser.id64 ].sharedconfig.UserRoamingConfigStore.Software.Valve.Steam.Apps = Object.assign({}, sc)
