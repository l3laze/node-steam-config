...
function steamPaths () {
  ...
  let obj = {
    ...
    get localconfig () {
      if (platform !== 'linux') {
        return path.join(rootPath, 'userdata', account, 'config', 'localconfig.vdf')
      } else {
        return path.join(rootPath, 'steam', 'userdata', account, 'config', 'localconfig.vdf')
      }
    },
    set localconfig (to) {},
    ...
    get sharedconfig () {
      if (platform !== 'linux') {
        return path.join(rootPath, 'userdata', account, '7', 'remote', 'sharedconfig.vdf')
      } else {
        return path.join(rootPath, 'steam', 'userdata', account, '7', 'remote', 'sharedconfig.vdf')
      }
    },
    set sharedconfig (to) {},
    ...
    get shortcuts () {
      if (platform !== 'linux') {
        return path.join(rootPath, 'userdata', account, 'config', 'shortcuts.vdf')
      } else {
        return path.join(rootPath, 'steam', 'userdata', account, 'config', 'shortcuts.vdf')
      }
    },
    set shortcuts (to) {},
