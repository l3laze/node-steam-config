...
async function requestWithCache (requestFunc, force, cache) {
  ...
  try {
    if (!force && cache.enabled && cache.folder && cache.file && fs.existsSync(path.join(cache.folder, cache.file))) {
      ...
      data = await requestFunc().catch((err) => {
...
async function requestOwnedApps (id64, force = false, cache = {enabled: false}) {
  ...
  let rf = async () => {
    ...
    data = FXP.parse(await data.text().catch((err) => {
      /* istanbul ignore next */
      throw err
...
async function requestGenres (appid, /* istanbul ignore next */ force = false, /* istanbul ignore next */ cache = {}) {
  ...
  try {
    ...
    if (typeof data === 'undefined') {
      console.info('Loading from internet.')
      data = await requestWithCache(reqFunc, force, {
        enabled: cache.enabled || /* istanbul ignore next */ false,
        folder: cache.folder || /* istanbul ignore next */ null,
        file: 'genres.json'
      })
