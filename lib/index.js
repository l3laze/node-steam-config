  ...
  switch (platform) {
    ...
    case 'win32':
      ...
      if (!fs.existsSync(detected)) {
        detected = (new Registry('HKLM\\Software\\Valve\\Steam')).get('InstallPath') || null
      }
      break
    ...
    for (let entry of entries) {
      ...
      switch (tmp) {
        case 'app':
          ...
          let filterFunc = (a) => a.AppState.appid === tmp2
          tmp = this.steamapps.filter(filterFunc)[ 0 ]
