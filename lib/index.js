...
SteamConfig.prototype.load = async function load (entries) {
  ...
  try {
    ...
    for (let entry of entries) {
      ...
      switch (tmp) {
        case 'app':
          data = await TVDF.parse('' + await afs.readFileAsync(entry))
        ...
        case 'shortcuts':
          if (fs.existsSync(entry)) {
            data = await BVDF.parseShortcuts(await afs.readFileAsync(entry))
    ...
    for (let entry of entries) {
      ...
      switch (tmp) {
        case 'app':
          ...
          await afs.writeFileAsync(entry, TVDF.stringify(tmp, true))
          break
        ...
        case 'registry':
          ...
          if (tmp !== 'winreg') {
            await afs.writeFileAsync(tmp, TVDF.stringify(this.registry, true))
          } else {
